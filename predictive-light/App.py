...
Microservizio FastAPI che espone GET /predict (ritorna {"predicted_ambient_light": <float>})
...
NOTA IMPORTANTE: questo servizio predice SOLO la luce ambientale futura.
Non decide MAI il valore di illuminazione dell'attuatore (brightness):
quella conversione (100 - luce_ambientale) e' responsabilita' esclusiva
del mash-up (telemetryPoller.js), che e' l'UNICO punto che decide cosa
va su regulateBrightness, sia in caso di successo che di fallback.

MODIFICA (metriche/test): oltre a InfluxDB, ogni riconciliazione viene
appesa a un CSV (prediction_errors.csv) usato dallo script di analisi
offline (analyze_results.py) per calcolare:
  - metrica #1: MAE / RMSE / bias residuo dell'errore di previsione ARIMA
  - metrica #2: tempo di convergenza del bias (EMA) dopo uno shock
"""
import os
import csv
import time
...
# ----------------------------- Logging CSV (metriche) -------------------
LOG_DIR = os.environ.get("LOG_DIR", "/app/logs")
os.makedirs(LOG_DIR, exist_ok=True)
PRED_ERROR_CSV = os.path.join(LOG_DIR, "prediction_errors.csv")
...
_init_csv(PRED_ERROR_CSV, ["timestamp", "predicted", "actual", "error", "abs_error", "ema_bias"])


def log_prediction_error_csv(target_ts: datetime, predicted: float, actual: float, error: float, ema_bias: float):
    """Appende una riga di riconciliazione al CSV usato da analyze_results.py
    per le metriche #1 (MAE/RMSE/bias) e #2 (convergenza EMA dopo shock).

    NB: 'predicted'/'actual'/'error' qui sono SEMPRE valori di luce
    ambientale (%), coerenti col resto del servizio: nessuna conversione
    verso brightness avviene in questo file.
    """
    try:
        with open(PRED_ERROR_CSV, "a", newline="") as f:
        ...
        logger.warning("Errore scrittura prediction_errors.csv: %s", exc)
...
class Predictor:
    ...
    Predice SEMPRE e SOLO il valore futuro di luce ambientale (%).
    Non ha alcuna conoscenza di "brightness" dell'attuatore.
    """
    def __init__(self):
        self.last_prediction: float | None = None  # ultima luce ambientale prevista (%)
    ...
    def fit_and_predict(self, series: list[tuple[datetime, float]]) -> float | None:
        ...
        # Clamp a [0, 100] (è una percentuale di luce ambientale)
        corrected = float(max(0.0, min(100.0, corrected)))

        self.last_prediction = corrected
    ...
    while True:
        try:
            ...
            for p in preds:
                ...
                # Metriche #1/#2: CSV con errore + EMA aggiornata a questa riconciliazione
                log_prediction_error_csv(p["target_timestamp"], p["predicted_value"], actual, err, predictor.ema_error)
...
async def predict():
    if predictor.last_prediction is None:
        ...
        # mashup usa il fallback reattivo (valore corrente del fotoresistore).
        from fastapi.responses import JSONResponse
        return JSONResponse(
            status_code=503,
            content={"error": "no prediction available yet"},
        )
    # NB: questo e' il valore PREVISTO di luce ambientale (%), NON un
    # valore di brightness. La conversione (100 - x) la fa il mash-up.
    return {"predicted_ambient_light": predictor.last_prediction}
